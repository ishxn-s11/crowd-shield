...
function poissonDisk(size: number, minD: number, maxD: number, tries: number, rand: () => number) {
...
export default function CursorRingField(props: CursorRingFieldProps) {
  ...
  const swatches = colors.slice(0, MAX_COLORS).map(hexToRgb);
  if (swatches.length === 0) swatches.push(hexToRgb("#7189ff"));
  ...
  useEffect(() => {
    ...
    try {
      ...
      const renderNames = [
        "uProjF", "uAspect", "uCamDist", "uPointScale",
    ...
    const frame = (now: number) => {
      ...
      if (densityDirty.current) { densityDirty.current = false; rebuildField(densityRef.current); }
      if (!count) return;
